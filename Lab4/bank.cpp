...
int main(){
    ...
    ba[0].setBalance((float)(rand() % 5000));
    ba[1].setBalance((float)(rand() % 5000));
    ba[2].setBalance((float)(rand() % 5000));
    for(int i = 0; i < ba.size(); i++){
        ba[i].setAccountNum(id);
        cout << ba[i];
        id++;
    }
    sa[0].setName("Xing");
    sa[1].setName("Yi");
    sa[2].setName("Zed");
    sa[0].setBalance((float)(rand() % 5000));
    sa[1].setBalance((float)(rand() % 5000));
    sa[2].setBalance((float)(rand() % 5000));
    for(int i = 0; i < ba.size(); i++){
        sa[i].setAccountNum(id);
        cout << sa[i];
        id++;
    }
